        ...
        <div class="font-thin text-sm">
          {#if data?.user?.isLoggedIn}
            <div class="flex items-center gap-2">
              {data?.user?.username}
              <button on:click={logoutHandler} class="hover:cursor-pointer">
                <svg
                  width="22px"
                  height="22px"
...
<footer>
  <div class="flex flex-1 justify-center px-5 py-6 bg-slate-900 text-white">
    <div
      class="flex flex-1 flex-col sm:flex-row items-center max-w-screen-xl justify-between font-thin text-sm text-center border-1 gap-2"
    >
      Copyright © 2023, ismail nural. All Rights Reserved
      <div class="font-thin text-sm">
